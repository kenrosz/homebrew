...
Check the API status: https://status.openai.com`),new S(s)}return JSON.parse(o)},eD=e=>e.trim().replace(/[\n\r]/g,"").replace(/(\w)\.$/,"$1"),tD=e=>Array.from(new Set(e)),Cn=async(e,t,n,u,r,o,s,i,D)=>{try{const l=await Qi(e,{model:t,messages:[{role:"system",content:Xi(n,o,s)},{role:"user",content:u}],temperature:.7,top_p:1,frequency_penalty:0,presence_penalty:0,max_tokens:200,stream:!1,n:r},i,D);return tD(l.choices.filter(f=>f.message?.content).map(f=>eD(f.message.content)))}catch(l){const f=l;throw f.code==="ENOTFOUND"?new S(`Error connecting to ${f.hostname} (${f.syscall}). Are you connected to the internet?`):f}};var uD=async(e,t,n,u,r)=>(async()=>{Yu(Lu(Nu(" aicommits "))),await Ju();const o=dt();n&&await te("git",["add","--update"]),o.start("Detecting staged files");const s=await Zu(t);if(!s)throw o.stop("Detecting staged files"),new S("No staged changes found. Stage your changes manually, or automatically stage all changes with the `--all` flag.");o.stop(`${pi(s.files)}:
${s.files.map(c=>`     ${c}`).join(`
`)}`);const{env:i}=process,D=await gt({OPENAI_KEY:i.OPENAI_KEY||i.OPENAI_API_KEY,proxy:i.https_proxy||i.HTTPS_PROXY||i.http_proxy||i.HTTP_PROXY,generate:e?.toString(),type:u?.toString()}),l=dt();l.start("The AI is analyzing your changes");let f;try{f=await Cn(D.OPENAI_KEY,D.model,D.locale,s.diff,D.generate,D["max-length"],D.type,D.timeout,D.proxy)}finally{l.stop("Changes analyzed")}if(f.length===0)throw new S("No commit messages were generated. Try again.");let a;if(f.length===1){[a]=f;const c=await fi({message:`Use this commit message?

   ${a}
`});if(!c||Hu(c)){V("Commit cancelled");return}}else{const c=await di({message:`Pick a commit message to use: ${ot("(Ctrl+c to exit)")}`,options:f.map(d=>({label:d,value:d}))});if(Hu(c)){V("Commit cancelled");return}a=c}await te("git",["commit","-m",a,...r]),V(`${we("\u2714")} Successfully committed!`)})().catch(o=>{V(`${be("\u2716")} ${o.message}`),$e(o),process.exit(1)});const[Ot,nD]=process.argv.slice(2);var rD=()=>(async()=>{if(!Ot)throw new S('Commit message file path is missing. This file should be called from the "prepare-commit-msg" git hook');if(nD)return;const e=await Zu();if(!e)return;Yu(Lu(Nu(" aicommits ")));const{env:t}=process,n=await gt({proxy:t.https_proxy||t.HTTPS_PROXY||t.http_proxy||t.HTTP_PROXY}),u=dt();u.start("The AI is analyzing your changes");let r;try{r=await Cn(n.OPENAI_KEY,n.model,n.locale,e.diff,n.generate,n["max-length"],n.type,n.timeout,n.proxy)}finally{u.stop("Changes analyzed")}const s=await O.readFile(Ot,"utf8")!=="",i=r.length>1;let D="";s&&(D=`# \u{1F916} AI generated commit${i?"s":""}
